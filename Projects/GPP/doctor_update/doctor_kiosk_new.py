...
class kiosk_doctor:
	...
	def on_reply_inbox_clicked(self,widget,data=None):
	 ...
	 #according to me when each and every time a function is called the vaalue of len_of_dir for both cf and ef should be recalculated
		if self.i >= self.len_of_dir_ef:
			
			if self.len_of_dir_cf==0:
				os.chdir("./download/other")#made changes here
				...
				os.chdir("./download/complaint")#made changes here
			...
			os.chdir("./download/emergency")#made changes here
	...
	def get_file_inbox_other(self):
		...
		if self.len_of_dir_other > 0:
			...
			# the time when the mail will be received and stored in the others directory will be considered as the filename
	...
	def on_cl_view_category_changed(self,widget,data=None):	
		...
		os.chdir("./Contact_Lists")#made changes here
		list_of_dir=os.listdir("./")
	...
	def on_inbox_delete_clicked(self,widget,data=None):
		...
		if self.filename==None:
			...
			if self.file_type=="complaint":
				...
				print "inside others delete now"
	...
	def on_cl_submit_clicked(self,widget,data=None):
		...
		os.chdir("./Templates")#kartik here
	...
	def on_cm_submit_clicked(self,widget,data=None):
		...
		buff=self.cm_textview.get_buffer()
		node=par.getElementsByTagName("details")
		if buff.get_text(buff.get_start_iter(),buff.get_end_iter(),include_hidden_chars=True)!="": #final changes
			print "value in details section"
			node[0].firstChild.nodeValue=buff.get_text(buff.get_start_iter(),buff.get_end_iter(),include_hidden_chars=True)
		else:
		        print "no value in details section"
			node[0].firstChild.nodeValue=None #final changes
			
		os.chdir("..")#kartik here
		...
		print "submit button of compose mail clicked"		
	...
	def __init__ (self):
		...
		os.chdir(dir_from_which_user_kiosk_will_take_files)
		os.chdir("./doctor_update/User_interface")#kartik here
